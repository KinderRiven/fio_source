			     ...
			     int ddir)
{
	...
	log_info("  %s: io=%s, bw=%s/s, iops=%s, runt=%6llumsec delay[%lld/%lld]\n",
...
void __show_run_stats(void)
{
	...
	last_ts = -1;
	idx = 0;
	//for(td = &(threads[0], i = 0; i < threads_num; i++, td++)
	for_each_td(td, i) {
		...
		last_ts = td->groupid;
		ts = &threadstats[j];
		ts->io_u_sum = td->io_u_sum;
		ts->io_u_usec = td->io_u_usec;
	...
	for (i = 0; i < nr_ts; i++) {
		...
		if (is_backend)
			...
			log_info("stat 1588:show thread status normal\n");
			show_thread_status_normal(ts, rs);
		}
	}
